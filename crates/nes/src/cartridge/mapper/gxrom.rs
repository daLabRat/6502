...
/// GxROM (Mapper 66) - 32KB PRG and 8KB CHR bank switching.
...
impl Mapper for GxRom {
    ...
    fn cpu_write(&mut self, addr: u16, val: u8) {
        if addr >= 0x8000 {
            self.prg_bank = ((val as usize >> 4) & 0x03) % self.prg_bank_count;
            self.chr_bank = (val as usize & 0x03) % self.chr_bank_count;
        }
    }
